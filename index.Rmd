...
bfi_a
```
### とにかく左側のdfに合わせる：left_join
```{r}
bfi_a_20_30 <- 
bfi_a %>% 
  slice(c(20:30)) %>% 
  select(id, A1, education)
...
# left_join(bfi_a_20_30, bfi_a_20_25, by = c("id"))
...
### 層別変数を関数内に組み込みver
```{r}
strat_freq <- function(df, outcome_var){
df %>%
  nest(data = -c(gender, education)) %>% # あらかじめ層別変数を指定しておく
  mutate(freq =
           map(data, ~ tabyl(.x, {{outcome_var}}, show_na = FALSE))) %>% # 欠損は除外して算出
  select(-data) %>% 
  unnest(cols = c(freq)) %>% 
  # drop_na(education) %>% 
  as_tibble()
...
```{r}
strat_freq <- function(df, outcome_var, group_var){
df %>%
  nest(data = -{{group_var}}) %>% 
  mutate(freq =
...
quos(c(gender,education)) %>% 
  map2(c("A1", "A2"), ~strat_freq(bfi, .y,  !!.x))
