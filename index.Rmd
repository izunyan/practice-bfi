...
bfi_a %>% 
  mutate(education2 = replace_na(education2, "unknown"), # 因子型には使えない？
              A1nm = replace_na(A1, "不明"),           # 文字型に
              A2nm = replace_na(A2, "不明"))           # 文字型に
  
bfi_anm %>% count(education2)
bfi_anm %>% count(A1nm)
```

### （動かないので修正中）特定の値を欠損値にする
```{r eval=FALSE, include=FALSE}
bfi_anm %>% 
...
df %>%
  ...
  mutate(freq =
           map(data, ~ tabyl(.x, {{outcome_var}}, show_na = FALSE))) %>% # show_na = FALSEで欠損無しのpercentのみ算出
  select(-data) %>% 
  unnest(cols = c(freq)) %>% 
  # drop_na(education) %>% 
  as_tibble()
...
df %>%
  nest(data = -{{nest_vars}}) %>% 
  mutate(freq =
           map(data, ~ tabyl(.x, {{outcome_var}}, show_na = FALSE))) %>% 
  select(-data) %>% 
  unnest(cols = c(freq)) %>% 
  # drop_na(education) %>% 
  as_tibble()
...
quos(c(gender,education)) %>% 
  map2(c("A1", "A2"), ~nest_tabyl(bfi, .y,  !!.x))

res_sf <- 
res_sf %>% 
  map(~drop_na(., education)) %>% 
  map(~arrange(., gender, education))
...
bfi_by_factor <- 
    map(factor_items, ~select(bfi_added, all_of(.)))
